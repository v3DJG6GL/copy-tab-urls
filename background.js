...
// Function to get the URL prefix from storage
async function getURLPrefix() {
  const result = await browser.storage.sync.get("urlPrefix");
  return result.urlPrefix || ""; // Return empty string if no prefix is set
}

async function copyUrl(sender, tabs) {
  const urls = tabs.map((tab) => {
    return tab.url;
  });

  if (urls.length > 0) {
    const urlPrefix = await getURLPrefix(); // Get the URL prefix
    const urlsWithPrefix = urls.map(url => urlPrefix + url); // Apply the prefix to each URL
    urls_str = urlsWithPrefix.join("\n");
